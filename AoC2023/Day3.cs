...
public sealed class Day3
{
    ...
    private static readonly Dictionary<(int, int), List<int>> _gears = new Dictionary<(int, int), List<int>>();
    private static readonly List<(int, int)> _removedGears = new List<(int, int)>();
    ...
    public static int SolveFirstChallenge(string input)
    {
        ...
        {
            sum += GetLineSum(i, lines);
        }

        return sum;
    }

    private static int GetLineSum(int lineIndex, string[] lines)
    {
        int sum = 0;
        ...
        {
            if (char.IsDigit(lines[lineIndex][i]))
            {
                sb.Append(lines[lineIndex][i]);
                ...
                validValue = IsValidValue(lines, lineIndex, i);
        ...
        if (validValue && int.TryParse(sb.ToString(), out int lastElement)) 
        {
            sum += lastElement;
        }
    ...
    //Method used under MIT license from Kossuranta
    ...
    private static bool IsSymbol(char arg)
    {
        return !char.IsDigit(arg) && arg != '.';
    }
    ...
    public static int SolveSecondChallenge(string input)
    {
        ...
        foreach (List<int> gearValues in _gears.Values) 
        {
    ...
    private static void FindGears(int lineIndex, string[] lines)
    {
        (int, int)? gear = null;
        StringBuilder sb = new StringBuilder();
        ...
        for (int i = 0; i < lines[lineIndex].Length; i++)
        {
            if (char.IsDigit(lines[lineIndex][i]))
            {
                ...
                gear = CheckForGearSymbol(lines, lineIndex, i);

                continue;
            }

            if (gear is not null && !_removedGears.Contains(gear.Value))
            {
                if (!_gears.ContainsKey(gear.Value))
                {
                    _gears[gear.Value] = new List<int>(2)
                    {
                ...
                if (_gears[gear.Value].Count == 2)
                {
                    _gears.Remove(gear.Value);
                    _removedGears.Add(gear.Value);
                    gear = null;
                    sb.Clear();
                    continue;
                }

                _gears[gear.Value].Add(int.Parse(sb.ToString()));
            }
        ...
        if (gear is not null && !_removedGears.Contains(gear.Value) && int.TryParse(sb.ToString(), out int lastElement))
        {
            if (!_gears.ContainsKey(gear.Value))
            {
                _gears[gear.Value] = new List<int>(2)
            ...
            _gears[gear.Value].Add(lastElement);
        }
    }

    public static (int, int)? CheckForGearSymbol(string[] lines, int lineIndex, int charIndex)
    {
        for (int li = lineIndex - 1; li < lineIndex + 2; li++)
        {
            if (li < 0 || li >= lines.Length)
