...
public sealed class Day2
{
    public const string TestInput = """
        ...
        """;
    public const string ChallengeInput = """
    ...
    public static int SolveFirstChallenge(string input, int maxGreen, int maxBlue, int maxRed)
    {
        int sum = 0;
        ...
        {
            bool possible = true;
            string[] gameVsDrawList = line.Split(": ");
            int currGameId = int.Parse(gameVsDrawList[0].Split(' ')[1]);
            string[] draws = gameVsDrawList[1].Split("; ");
            foreach (string draw in draws)
            {
                ...
                {
                    string[] colorNumberAndText = color.Split(" ");
                    int number = int.Parse(colorNumberAndText[0]);

                    switch (colorNumberAndText[1])
                    {
                        case "green":
                            if (number > maxGreen)
                            {
